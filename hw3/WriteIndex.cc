...
static int WriteHeader(FILE *f, int doctable_bytes, int memidx_bytes) {
  ...
  char buf[BUF];
  uint8_t fsize;
  size_t res, bytes_read = 0;

  // fseek to end of file to obtain size of file
  ...
  // rewind to begining of file to be read
  rewind(f);
  // read to end of file
  while ((res = fread(buf, BUF, 1, f)) != 0) {
    // calculate checksum for each byte read
    for (int i = 0; i < (int) res; i++) {
      crc.FoldByteIntoCRC((uint8_t) buf[i]);
    }
    bytes_read += res;
  }
...
static int WriteHashTable(FILE *f, IndexFileOffset_t offset, HashTable *ht,
  ...
  for (int i = 0; i < ht->num_buckets; i++) {
    // STEP 4.
    LinkedList *buckets = ht->buckets[i]; // not accessing ht correctly
    int32_t num_elem = LinkedList_NumElements(buckets);
    record_pos += WriteHTBucketRecord(f, record_pos, num_elem, bucket_pos);
    bucket_pos += WriteHTBucket(f, bucket_pos, buckets, fn);
  }
...
static int WriteHTBucket(FILE *f, IndexFileOffset_t offset, LinkedList *li,
  ...
  for (int i = 0; i < num_elts; i++) {
    ...
    ElementPositionRecord epr = (ElementPositionRecord) record_pos;
    epr. ToDiskFormat();
    ...
    LLPayload_t payload = new LLPayload_t();
    LLIterator_Get(it, &payload);
    Verify333(payload != nullptr);
    fn(f, element_pos, (HTKeyValue_t*) payload); // passing payload incorrectly 
...
static int WriteDocidToDocnameFn(FILE *f, IndexFileOffset_t offset,
  ...
  int16_t file_name_bytes = (int16_t) strlen(reinterpret_cast<char*>(kv->value)); // length of the name of the file
  ...
  if (fseek(f, sizeof(DoctableElementHeader), SEEK_CUR) != 0) {
    return kFailedWrite;
  }
  if (fwrite(&kv->value, file_name_bytes - 1, 1, f) != 1) {
    return kFailedWrite;
  }
...
static int WriteDocIDToPositionListFn(FILE *f,
  ...
  DocIDElementHeader header(doc_id, (int32_t) num_positions);
  header.ToDiskFormat();
  ...
  for (int i = 0; i < num_positions; i++) {
    ...
    position.position = (uint32_t) reinterpret_cast<int64_t>(&position.position);
  ...
  return sizeof(DocIDElementHeader);  // you may need to change this return value
}
...
static int WriteWordToPostingsFn(FILE *f,
  ...
  int16_t word_bytes = (int16_t) strlen(wp->word);  // you may need to change this logic
  ...
  header.ToDiskFormat();
  if (fseek(f, ht_bytes, SEEK_SET) != 0) {
    return kFailedWrite;
  }
  ...
  // place in the file.
  if (fseek(f, sizeof(WordPostingsHeader), SEEK_CUR) != 0) {
    return kFailedWrite;
  }
  if (fwrite(wp->word, word_bytes - 1, 1, f) != 1 ) {
    return kFailedWrite;
  }
