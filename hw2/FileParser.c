...
char* ReadFileToString(const char *file_name, int *size) {
  ...
  if (!S_ISREG(file_stat.st_mode)) {
    // handle file open err - not a txt file
    fprintf(stderr, "Error on acquiring file stats. Try new file. \n");
    return NULL;
  }
  ...
  *size = file_stat.st_size;
  ...
  if (buf == NULL) {
    fprintf(stderr, "Error allocating space for the file. \n");
    close(fd);
    return NULL;
  }
...
static void InsertContent(HashTable *tab, char *content) {
  ...
  //  declare to keep track of wordstart position
  DocPositionOffset_t pos = 0;
  //  parse through content to find all words
  while (*cur_ptr != '\0') {
    //  if current character is alphabetic 
    if (isalpha(*cur_ptr)) {
      //  make lower case
      *cur_ptr = tolower(*cur_ptr);
      //  if wordstart is null, update to start of new word
      //  and update position of wordstart
      if (word_start == NULL) {
        word_start = cur_ptr;
        pos = cur_ptr - content;
      }
    } else {
      //  null terminate the non-alphabetic character
      *cur_ptr = '\0';
      //  if wordstart isn't null, found end of word and should add to table
      if (word_start != NULL) {
  ...
  //  handle case of possible last word
  if (word_start != NULL) {
    *cur_ptr = '\0';
    AddWordPosition(tab, word_start, pos);
    word_start = NULL;
...
static void AddWordPosition(HashTable *tab, char *word,
  ...
  if (HashTable_Find(tab, hash_key, &kv)) {
    ...
    //  allocate space for new WordPositions struct
    wp = (WordPositions*) malloc(sizeof(WordPositions));

    //  set value of new word
    wp->word = strdup(word);
    //  allocatre new linked list for positions of new word
    wp->positions = LinkedList_Allocate();
    //  appened position to new linked list
    LinkedList_Append(wp->positions, (LLPayload_t) (int64_t) pos);

    //  set new key
    kv.key = hash_key;
    //  set WordPosition to new value
