...
int a_sort(const void *v1, const void *v2) {
  return alphasort((const struct dirent**) v1, (const struct dirent**) v2);
}
...
static void HandleDir(char *dirpath, DIR *d, DocTable **doctable,
  ...
  num_entries = i;
  qsort(entries, num_entries, sizeof(struct entry_st), &a_sort);
